    ...
    (labels
      ...
      (macrolet ((match-avoiding-walker-loops (subform &rest clauses)
        ...
        (labels
            ((alpha-convert (subform context env)
               ...
               (match-avoiding-walker-loops subform
                  ((list* 'LAMBDA args body)
                   ;;
                   ;; Here we alpha convert lambda args in case one
                   ;; or more of them identify with global special
                   ;; symbols.  Failing to do this step means that
          ...
          (let ((expansion (walker:walk-form form env #'alpha-convert)))
            (when dbg
