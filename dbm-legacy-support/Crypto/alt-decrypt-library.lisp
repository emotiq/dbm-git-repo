...
;; Decompressor module
(defvar *decompressor*   ".T{809C748C-5E32-11E1-9D35-C82A14446EA7}")
...
(defun forced-honesty-path ()
  (if *live-decryption*
      (merge-pathnames *forced-honesty* (exec-directory))
    (merge-pathnames *forced-honesty* *auhost-path*)))

(defun decompressor-path ()
  (if *live-decryption*
      (merge-pathnames *decompressor* (exec-directory))
    (merge-pathnames *decompressor* *auhost-path*)))
...
(defun decrypt-library (filename-in filename-out bundle-id)
  (multiple-value-bind (user-uuid unlock-share)
      (read-user-license filename-out)
    (let* ((unlock-pt     (compute-product-unlock-pt user-uuid unlock-share))
        ...
        (progn
          ...
          (destructuring-bind (progname exename fromfile tofile bundle-id)
            ...
            (let ((*live-decryption* exename))
              #|
              (format t "fo path:   ~A~%" (forced-honesty-path))
              (format t "decompressor: ~A~%" (decompressor-path))
              ...
              |#
              ;; (print "Hello from alt-decrypt-library")
