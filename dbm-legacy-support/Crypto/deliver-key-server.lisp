...
./Lispworks-6-0-0-macos64-universal -build ~/projects/lispworks/VTuning/crypto/tools/deliver-key-server.lisp
popd
...
"c:/program Files/Lispworks/lispworks-6-0-0-x64-windows.exe" -init ./VTuning/crypto/tools/deliver-key-server.lisp
popd
...
(load "dongle")
(asdf "butterfly")
;; (load "Butterfly/Examples/Name-Server")
(asdf "ecc-messaging")
(load "VTuning/crypto/tools/key-server")
...
(deliver 'ecc-crypto-b571::start-key-server

         #+:MACOSX
         (let ((this-dir (translate-logical-pathname "PROJECTS:LISP;VTuning;crypto;tools;")))
           (write-macos-application-bundle
            "/Applications/Acudora Key Server.app"
            :signature  "ACUD"
            :identifier "com.acudora.ecc-ibe-key-server"
            :application-icns (merge-pathnames "calculator.icns" this-dir)
            :document-types nil))

	 #+:WIN32 "Acudora Key Server.exe"
	 ...
	 :versioninfo
	  ...
	  :product-name   "Acudora ECC Messaging"
	  :file-description "Use to encrypt/decrypt ECC messages"
